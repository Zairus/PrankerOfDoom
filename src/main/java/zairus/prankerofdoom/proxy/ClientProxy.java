package zairus.prankerofdoom.proxy;
...
import zairus.prankerofdoom.PODConstants;
import zairus.prankerofdoom.client.renderer.ISpecialRendered;
...
public class ClientProxy extends CommonProxy
{
	...
	public void registerModel(Item item, int meta, String name)
	{
		...
		ModelResourceLocation modelResourceLocation = new ModelResourceLocation(PODConstants.MODID + ":" + name, "inventory");
		
		renderItem.getItemModelMesher().register(item, meta, modelResourceLocation);
		
		if (meta == 0)
			ModelBakery.registerItemVariants(item, new ResourceLocation(PODConstants.MODID, name));
	}
